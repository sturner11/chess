    ...
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition position) {
        ...
        if (myBoard[oRow - 1][oCol - 1].getTeamColor() == ChessGame.TeamColor.WHITE) {
            if (oCol - 2 >= 0 && myBoard[oRow][oCol - 2] != null && myBoard[oRow][oCol - 2].getTeamColor() != piece.getTeamColor()) {
                ChessMove move;
                if (oRow + 1 == 8) {
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol - 1), ChessPiece.PieceType.ROOK);
                    pieceMoves.add(move);
                    ...
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol - 1), null);
                }
                pieceMoves.add(move);
            }
            if (myBoard[oRow][oCol - 1] == null) {
                ...
                ChessMove move;
                if (oRow + 1 == 8) {
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol), ChessPiece.PieceType.ROOK);
                    pieceMoves.add(move);
                    ...
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol), null);
                }
                pieceMoves.add(move);
            }
            if (oCol + 2 < 8 && myBoard[oRow][oCol] != null && myBoard[oRow][oCol].getTeamColor() != piece.getTeamColor()) {
                ChessMove move;
                if (oRow + 1 == 8) {
                    move = new ChessMove(position, new ChessPosition(oRow, oCol + 1), ChessPiece.PieceType.ROOK);
                    pieceMoves.add(move);
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol + 1), ChessPiece.PieceType.KNIGHT);
                    pieceMoves.add(move);
                    move = new ChessMove(position, new ChessPosition(oRow + 1, oCol + 1), ChessPiece.PieceType.BISHOP);
                    ...
                    move = new ChessMove(position, new ChessPosition(oRow, oCol + 1), null);
                }
                pieceMoves.add(move);
            }
        } else { //BLACK
            pieceMoves.addAll(blackMoves(position, myBoard, oRow, oCol, piece));
        }
        return pieceMoves;
    }

    public Collection<ChessMove> blackMoves(ChessPosition position, ChessPiece[][] myBoard, int oRow, int oCol, ChessPiece piece) {
